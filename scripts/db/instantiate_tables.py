...
def instantiate_tables():
    ...
    with psycopg2.connect(dbname=url.path[1:],user=url.username,password=url.password,host=url.hostname,port=url.port) as conn:
        with conn.cursor() as cursor:
            cursor.execute("CREATE TABLE IF NOT EXISTS xbox_status (id BIGINT PRIMARY KEY, xbox_ids BIGINT[], names TEXT[])")
            cursor.execute("CREATE TABLE IF NOT EXISTS playstation_status (id BIGINT PRIMARY KEY, playstation_ids BIGINT[], names TEXT[])")
            ...
            cursor.execute("CREATE TABLE IF NOT EXISTS notifications (id UUID PRIMARY KEY, message TEXT, users TEXT[])")
